...
#
# The topology, its coordinates and the 200 Map Ta Phut customer ids are SIMULATED —
# generated by `scripts/seed_db.py` (PR-I), with no real customer PII. Only `branch` comes
# from the real curated roster. Coordinates are SCHEMATIC (a process diagram), not GIS.
...
class TypeBreakdown(BaseModel):
    """Distinct affected accounts per PWA top-level customer type (PR-I, item 2.4).

    1 = residential/other, 2 = government/small business, 3 = state enterprise/industry/large
    business. The three sum to `ImpactResponse.count`.
    """
...
class AffectedCustomer(BaseModel):
    ...
    The enriched fields are populated only when the Map Ta Phut impact feature is on
    (`MTP_CUSTOMER_IMPACT_ENABLED`); otherwise they are `null` (PR-I contract "null when off").
    """
    ...
    latest_usage_m3: int | None = None


class ImpactResponse(BaseModel):
    """Who loses water when `pipe_id` fails (scored item 2.4).

    `zone` and `type_breakdown` are populated only when the Map Ta Phut impact feature is on;
    otherwise `null`. The count is always the number of distinct affected service points.
    """
    ...
    count: int = 0
    simulated: bool = True
    #: The simulated low-pressure zone id (e.g. "MTP-LPZ-01"); null when the feature is off.
    zone: str | None = None
    #: Distinct accounts per top-level type; null when the feature is off.
    type_breakdown: TypeBreakdown | None = None


class DemoMeterReading(BaseModel):
...
class DemoCustomerDetail(BaseModel):
    """One SIMULATED Map Ta Phut account plus its 12-month reading history (PR-I).

    Served by `GET /api/twin/customers/{customer_id}` only when the impact feature is on. Every
    field is synthetic (`SIM-MTP-*`, `จำลอง` address); no real customer data exists here.
    """
    ...
    branch: str
    profile_version: str
    readings: list[DemoMeterReading] = Field(default_factory=list)
    simulated: bool = True


class ImpactZoneFeature(BaseModel):
    """One GeoJSON feature of the simulated low-pressure footprint."""
...
class ImpactZoneCollection(BaseModel):
    """The simulated low-pressure footprint served by `GET /api/twin/gis/impact-zones`.

    A coarse polygon around the demo Map Ta Phut zone — a geographic AREA, never a customer
    point. `provenance` is pinned SIMULATED so this can never be mistaken for real GIS geometry.
    """

    type: Literal["FeatureCollection"]
    scenario_id: str
    zone_id: str
    provenance: Literal["SIMULATED_LOW_PRESSURE_FOOTPRINT"]
