...
| 2.2 | Device status auto-updates, no refresh (5) | ✅ (P-2 red at cold start) | PR-7 | `topic2 2.2` |
...
> in a clock-hour, so P-2 reads red only near a **true cold start** (`make demo-down` then preflight);
> after the sim runs a while, live normal-mode telemetry washes it toward `warning`. Run the twin items
> shortly after preflight.
